...
<body>
    <div id="toolbar">
        <div class="group-box">
            <strong>Normalized time control:</strong>
            <label>Window(s) <input id="window-len" type="number" min="0.1" value="10"></label>
            <label>Progress <input id="start-slider" type="range" min="0" max="100" value="0" step="0.1"></label>
            <input id="time-start" type="text" placeholder="Start">
            <span class="sep">~</span>
            <input id="time-end" type="text" placeholder="End">
            <button id="apply-time-btn">Jump</button>
            <span id="current-window"></span>
        </div>
        <button id="settings-toggle">Channel Y range</button>
    </div>

    <div id="settings-panel">
        <div class="panel-header">
            <span>Per-channel Y range</span>
            <button id="settings-close" style="background:none;color:#333;font-size:16px;padding:0;">&times;</button>
        </div>
        <div class="panel-content">
            <div class="batch-control">
                <label>Apply the same range to all channels:</label>
                <div class="batch-control-row">
                    ...
                    <button id="apply-batch-btn" style="font-size:12px;">Apply</button>
                    <button id="reset-all-btn" style="font-size:12px; background:#64748b;">Reset</button>
                </div>
                ...
                <div class="batch-control-row">
                    ...
                    <button id="reset-all-colors-btn" style="font-size:12px; background:#64748b;">Default colors</button>
                </div>
            </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            function updateFromSlider() {
                ...
                currentWindow.textContent = "Current window: " + startText + " ~ " + endText;
            ...
            function applyRangeFromInputs() {
                ...
                if (startMs === null || endMs === null) {
                    alert("Time format must be HH:MM:SS, for example 12:34:56");
                    return;
                }
                if (startMs >= endMs) {
            ...
            function applySingleChannel(index) {
                ...
                if (!minValue || !maxValue) {
                    alert("Please fill both min and max");
                    return;
                }
            ...
            function applyBatch() {
                ...
                if (!minValue || !maxValue) {
                    alert("Please enter batch min and max");
                    return;
                }
...
def build_channel_controls(signal_cols) -> str:
    ...
    for index, col in enumerate(signal_cols):
        ...
        <div class="channel-row">
            ...
            <input id="y-min-{index}" type="number" step="0.1" placeholder="Min">
            <span class="sep">~</span>
            <input id="y-max-{index}" type="number" step="0.1" placeholder="Max">
            <button class="apply-btn-small" onclick="applySingleChannel({index})">Apply</button>
            ...
            <button class="apply-btn-small" onclick="applySingleChannelColor({index})">Color</button>
        </div>
            """.format(
...
def process_file(file_path: Path, overwrite: bool = False) -> str:
    ...
    try:
        ...
        fig, signal_cols = build_figure(df, f"{file_path.stem} (Normalized, multi-Y)")
        fig_html = inject_div_id(pio.to_html(fig, include_plotlyjs="cdn", full_html=False))
        output_path.write_text(
            build_page_html(fig_html, f"{file_path.stem} (Normalized) - multiY", signal_cols),
            encoding="utf-8",
        )
...
def main() -> None:
    ...
    print(
        "Normalized fixed-duration multiY chart generation complete. "
